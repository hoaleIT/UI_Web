...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h3 className="text-2xl font-bold text-foreground">Thống kê tổng hợp</h3>
        <p className="text-muted-foreground">Báo cáo học tập năm học 2024-2025</p>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-success">+12% so với năm trước</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-success">+5% so với năm trước</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-success">+0.3 so với năm trước</p>
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              <div>
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span>Giỏi (GPA {'>'}= 8.0)</span>
                  <span className="font-medium text-success">{excellentStudents} học sinh ({Math.round(excellentStudents/students.length*100)}%)</span>
                </div>
              ...
              <div>
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span>Khá (GPA 6.5 - 7.9)</span>
                  <span className="font-medium text-warning">{goodStudents} học sinh ({Math.round(goodStudents/students.length*100)}%)</span>
                </div>
                <div className="h-3 w-full overflow-hidden rounded-full bg-accent">
                  <div 
              ...
              <div>
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span>Trung bình (GPA {'<'} 6.5)</span>
                  <span className="font-medium text-destructive">{avgStudents} học sinh ({Math.round(avgStudents/students.length*100)}%)</span>
                </div>
                <div className="h-3 w-full overflow-hidden rounded-full bg-accent">
                  <div 
        ...
        <Card>
          <CardHeader>
            <CardTitle>Sĩ số theo khối</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {[12, 11, 10].map((grade) => {
                ...
                return (
                  <div key={grade}>
                    <div className="mb-1 flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">{totalStudents} học sinh ({gradeClasses.length} lớp)</span>
                    </div>
                    <div className="h-3 w-full overflow-hidden rounded-full bg-accent">
                      <div 
